    ...
    try {
        ...
        if (!existingStack) {
            return res.status(404).json({
                success: false,
                error: 'Distributed stack not found',
            });
        }
        ...
        // Log the distribution item update
        await auditLogger.log({
            ...
            details: `Updated distribution item ${result.item.name} (Quantity: ${result.quantity})`,
            metadata: {
                action: 'distribution_item_updated',
        ...
        if (error.code === 'P2025') {
            return res.status(404).json({
                success: false,
                error: 'Distribution item or stack not found',
            });
        }
