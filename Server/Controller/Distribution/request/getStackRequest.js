...
async function getStackRequests(req, res) {
    try {
        ...
        // Get all item transactions for the specific item - ONLY for Distribution stacks
        ...
        return res.status(200).json({
            success: true,
            message: 'Distribution item requests retrieved successfully',
            count: transformedRequests.length,
            itemId: itemId,
            requests: transformedRequests,
        });
    } catch (error) {
        console.error('Error fetching distribution item requests:', error);
